...
var Hud = function(game, width, height, title = null, location = null) {
  ...
  this.base_x = 50; // base x of where to draw hud
  ...
  this.title_text = "D'Knight"; // title text
  ...
  this.game.load.bitmapFont(
    "mainFont",
    "assets/fonts/ganonwhite/font.png",
    "assets/fonts/ganonwhite/font.xml"
  );
...
Hud.prototype.printItems = function() {
  for (i = 0; i < this.items.length; i++) {
    ...
    item = this.game.add.text(
      0,
      0,
      key + ":" + value,
      this._getStyle("#FFF", 14)
    );
    item.setShadow(3, 3, "rgba(0,0,0,0.5)", 2);
    item.setTextBounds(
      this.game.camera.x,
      this.game.camera.y + (i + 1) * 20,
      this.hud_width,
      this.hud_height
    );
...
Hud.prototype.addTitle = function(title = null) {
  ...
  title = this.game.add.text(
    ...
    75,
    this.title_text,
    this._getStyle("#FFF", this.font_size, true)
  );
  title.setShadow(3, 20, "rgba(0,0,0,10)", 2);
  title.setTextBounds(
    this.game.camera.x,
    this.game.camera.y,
    this.hud_width,
